...
\begin{itemize}
    ...
    \item \textbf{Conjunto de caracteres:} 62 caracteres no total, incluindo:
    \begin{itemize}
        \item 52 letras (26 minúsculas + 26 maiúsculas)
        \item 10 dígitos (0-9)
    \end{itemize}
    \item \textbf{Espaço de busca:} $62^{10} \approx 8.39 \times 10^{17}$ combinações possíveis
\end{itemize}
...
A estratégia de balanceamento de carga entre threads utiliza uma abordagem de intervalos contínuos. Para um dado comprimento de senha $l$, o número total de combinações é $total = 62^l$. Cada thread com identificador $id$ (de 0 a $n-1$, onde $n$ é o número de threads) recebe um intervalo:
...
Para gerar uma senha candidata a partir de um índice numérico, utilizamos uma técnica de conversão de base. O índice é interpretado como um número em base $CHARSET\_SIZE$ (62), onde cada dígito corresponde a um caractere do conjunto permitido.
...
\begin{enumerate}
    ...
    \item O caractere na posição $i$ é determinado por $charset[index \mod 62]$
    \item O índice é atualizado para $index \div 62$
...
A ordem implementada é: $\{8, 7, 9, 10, 6, 5, 4, 3, 2, 1\}$, baseada em estudos empíricos que mostram que senhas de 6 a 10 caracteres são mais comuns em datasets de senhas vazadas.
...
\begin{itemize}
    \item \texttt{md5crack-sequential.c}: Implementação sequencial standalone usando OpenSSL EVP
    \item \texttt{md5crack-concurrent.c}: Implementação concorrente usando POSIX Threads (pthread) e OpenSSL EVP
    \item \texttt{md5crack-cuda.cu}: Implementação paralela usando CUDA com MD5 implementado diretamente na GPU
    \item \texttt{md5crack.c}: Versão combinada original com suporte a ambos os modos (sequencial e concorrente)
...
Calcula o número total de combinações possíveis para um dado comprimento: $62^l$.
...
\begin{itemize}
    ...
    \item \textbf{Grid/Block configuration:} Configuração de 1024 blocos com 256 threads cada (262,144 threads simultâneas)
\end{itemize}
...
\begin{tabular}{|c|c|}
 ...
 4aef3d4d3dd9e36d3042309458b04314 & senhaL3gal \\
 d6fe902a1d89be1731428be31caf9586 & xXs3nh4Xx \\
 f8772716367e5a3b5145fff616c27ba1 & 0028122004 \\
...
Os testes foram executados em dois sistema com as seguintes características:
\begin{itemize}
    \item Sistema operacional: Windows
    \item Versões testadas:  CUDA (262,144 threads)
    \item Otimização: -O3 para versão CUDA
    \item Devido ao alto tempo de execução foi executado 1 vez para cada teste.
    \item Versão CUDA 13.0 e GPU NVIDIA GTX 1660 8GB 
\end{itemize}
\begin{itemize}
    \item \textbf{TODO:} PREENCHER AS CONFIGS DO SISTEMA UTILIZADO
    \item Sistema operacional: Linux
    \item Versões testadas: Sequencial (1 thread), Concorrente pthread (8 threads), CUDA (262,144 threads)
    \item Otimização: -O3 para versões CPU, -O3 para versão CUDA
    \item Cada teste foi executado múltiplas vezes para obter médias confiáveis
    \item Versão CUDA requer GPU NVIDIA compatível (informações específicas do hardware devem ser documentadas)
\end{itemize}

\subsection{Resultados de Corretude}

Todos os 5 casos de teste de corretude foram executados com sucesso nas três versões implementadas: sequencial, concorrente (pthread com 8 threads) e CUDA. O programa sempre encontrou a senha correta correspondente ao hash fornecido, validando a corretude de todas as implementações, incluindo a implementação própria do MD5 na versão CUDA.

\subsection{Resultados de Desempenho}

\textbf{TODO:} ADICIONAR OS VALORES DE TESTE.
\begin{itemize}
    \item Tabelas comparando tempo de execução entre versão sequencial, concorrente (pthread) e CUDA
    \item Gráficos mostrando speedup em função do número de threads
    \item Análise do overhead de sincronização
    \item Comparação entre execução com e sem análise de frequência
    \item Análise de eficiência entre CPU (pthread) e GPU (CUDA)
\end{itemize}

\textbf{Exemplo de estrutura de dados a coletar:}

\begin{center}
\begin{tabular}{|c|c|c|c|c|}
 \hline
 \textbf{Caso de Teste} & \textbf{Sequencial (s)} & \textbf{Concorrente (s)} & \textbf{CUDA (s)} & \textbf{Speedup CUDA} \\
 \hline
 c0r1nTh14n & --- & --- & --- & --- \\
 senhaL3g@l & --- & --- & --- & --- \\
 x\_s3nh@\_x & --- & --- & --- & --- \\
...
\textbf{TODO:} INCLUIR ANALISE DOS RESULTADOS:

\begin{itemize}
    \item Eficiência da paralelização (speedup alcançado vs. speedup teórico)
    \item Identificação de gargalos
    \item Comportamento com diferentes números de threads
    \item Impacto da análise de frequência no tempo médio de busca
\end{itemize}

\section{Dificuldades e Desafios Encontrados}
